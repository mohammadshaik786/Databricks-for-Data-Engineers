...
sourceFile = "/mnt/training/dataframes/people-with-dups.txt"
destFile = userhome + "/people.parquet"
...
# # df.groupBy("firstName","middleName","lastName")


# COMMAND ----------

# From above only the the records which are case sensitive are dropped, but we want records which are not case sensitive should be deleted.
# The same with SSN, firstly converting it into actual format and then removing duplicates.

# COMMAND ----------

# dedupDF = df.select(col("*"),
#                     lower(col("firstName")).alias("lcFirstName"),
#                     lower(col("lastName")).alias("lcLastName"),
#                     lower(col("middleName")).alias("lcMiddleName"),
#                     translate(col("ssn"), "-", "").alias("ssnNums")
#                    ).dropDuplicates(["lcFirstName","lcMiddleName","lcLastName","ssnNums"]).drop("lcFirstName", "lcMiddleName", "lcLastName","ssnNums")

# COMMAND ----------

dedupDF2 = df.withColumn("firstName", lower(df["firstName"]))\
